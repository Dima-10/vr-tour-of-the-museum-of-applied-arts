...
// Кастомная функция createInfoHotspotElement()
  function createInfoHotspotElement(hotspot) {
  ...
  console.log('Создаём попап:', hotspot);
  ...
  // вертикальная/горизонтальная ориентация картинки -------------------------
  const imgEl = popup.querySelector('.popup-image');
  const popupBody = popup.querySelector('.popup-body');
  const popupText = popup.querySelector('.popup-text');
  const popupHeader = popup.querySelector('.popup-header');
  
  // Проверка ориентации и установка классов
  function applyPortraitLayout(image) {
    const isPortrait = image.naturalHeight > image.naturalWidth;
  
    if (isPortrait) {
      image.classList.add('portrait');
      popupBody.classList.add('horizontal');
      popup.classList.add('portrait');
      popupText.classList.add('portrait');
      popupHeader.classList.add('portrait');
  ...
  // При повторной загрузке (например, из кэша)
  if (imgEl.complete) {
    applyPortraitLayout(imgEl);
  } else {
    imgEl.onload = () => applyPortraitLayout(imgEl);
  }

  // попап увеличения ------------------------------------------------
  const img = popup.querySelector('.popup-image');
  const zoomOverlay = popup.querySelector('.popup-zoom-overlay');
  const zoomImage = popup.querySelector('.popup-zoom-image');
  ...
  zoomOverlay.addEventListener('wheel', (e) => {
    ...
    scale = Math.min(Math.max(scale + delta, 1), 5); // от 1 до 5
    updateTransform();
  }, { passive: false });
  ...
  function updateTransform() {
    zoomImage.style.transform = `scale(${scale}) translate(${translate.x}px, ${translate.y}px)`;
  }
